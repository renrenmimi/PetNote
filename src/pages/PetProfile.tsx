    ...
    <div className="min-h-screen bg-white pb-10">
      <header className="sticky top-0 z-10 border-b border-slate-200 bg-white">
        <div className="mx-auto flex w-full max-w-md items-center justify-between px-4 py-3">
          ...
          <h1 className="text-base font-semibold text-slate-900">Pet Profile</h1>
          <div className="w-6" />
        </div>
      </header>
      ...
      <main className="mx-auto w-full max-w-md space-y-6 px-4 py-6">
        <section className="rounded-3xl bg-white p-6 text-center shadow-[0_18px_40px_-28px_rgba(15,23,42,0.4)] ring-1 ring-slate-100">
          <div className={`mx-auto w-fit rounded-full bg-gradient-to-r ${speciesMeta.gradient} p-1`}>
            {pet.avatarUrl ? (
              ...
              <div className="flex h-24 w-24 items-center justify-center rounded-full border-4 border-white bg-white text-4xl">
                {speciesMeta.emoji}
              </div>
            )}
          </div>
          ...
          {pet.bio ? (
            <p className="mt-3 text-sm text-slate-600">{pet.bio}</p>
          ) : null}

          <button
            type="button"
            onClick={() => navigate(`/profile/${pet.ownerId}`)}
            className="mt-3 text-xs text-slate-500 hover:text-purple-600"
          >
            Owner: {ownerName}
          </button>
        ...
        <section className="rounded-2xl bg-white p-4 text-center shadow-sm ring-1 ring-slate-100">
