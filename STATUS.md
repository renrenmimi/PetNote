...
| #208 | `stack-2-core` `6dba377` | 在（两次修复都落在这一层：`059db25`、`e6a8eb8`） | 最新两次都通过（294 条）。本轮更早的运行红过：点赞测试、夹具测试、第二条断流路径、检查门误报、媒体服务器探测，原因和处理见下两节 |
| #209 | `stack-3-models` `0e27708` | 在（从 stack 2 合并上来） | 最新两次都通过（544 条）；更早一次红在等 0.5 秒的评论测试上 |
| #210 / #204 | `feature/ios-native-prototype` `b5b273f` | 在 | 最新三次都通过（745 条）；更早红过：整图测试、一次测试进程崩溃、一次媒体服务器探测 |
...
### 候选包（2026-09-23，`190d6fe` 的代码）

`scripts/build-testcloud-package.sh --release-check`，退出码 0：
