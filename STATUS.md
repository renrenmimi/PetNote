...
| #208 | `stack-2-core` `2679f33` | 在（两次修复都落在这一层：`059db25`、`e6a8eb8`） | `5a881c0` 通过（run 35927282455，含对照）；`2679f33`（数计时器前先等它就位）待看。这一层本轮新增：CI 控制步骤和机器记录（`7f30afa`）、视频测试测量（`3f3fb95`）、测试超时的两处时机修正（`5a881c0`、`2679f33`） |
| #209 | `stack-3-models` `e9fc46d` | 在（从 stack 2 合并上来） | `6965599` 通过（run 35927281664，含对照）；`e9fc46d` 待看。这一层本轮新增：宠物替身加锁（`352fd8e`） |
| #210 / #204 | `feature/ios-native-prototype`（本次推送后的顶端） | 在 | `3337439` 两次都通过（run 35927282957、35927281157，含对照）；`be9ce4d` 两次里 1 次红在点赞测试上（数计时器太早，已修）；最新顶端待看。之前 `44f8d99`、`3b4f497` 各红一次，原因都已查明并修复，见「CI 上的另外两类红」 |
