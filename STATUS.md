...
| #209 | `stack-3-models` `f78831d` | 在（从 stack 2 合并上来） | `e9fc46d` 通过（run 35931481705，含对照）；`f78831d`（编辑资料连点测试）待看。这一层本轮新增：宠物替身加锁（`352fd8e`）、编辑资料连点测试（`f78831d`） |
| #210 / #204 | `feature/ios-native-prototype`（本次推送后的顶端） | 在 | `045213c` 两次都通过（run 35931482008、35931481006，含对照）；`5baad70` 两次里 1 次红在编辑资料连点测试上（测试没有真正制造重叠，已修）；最新顶端待看 |
...
- **编辑资料的「连点两下只保存一次」测试红一次（`5baad70`，run 35933245850；这个提交也只改了状态表）**：服务端被写了 2 次。测试用 `async let` 同时发两次保存，但假服务端瞬间返回，慢机器上第一次保存**已经做完**，第二次才开始——不重叠，写两次是对的。App 的防重复（开始保存前同步置位）根本没被这次运行测到。改成让第一次保存停在服务端、确认它在进行中再点第二下、然后放行（`bc01248`，stack 3 上是 `f78831d`）。**反向验证**：临时去掉 App 里的防重复，这条测试失败（写了 2 次）；恢复后 806 条串行全过。注册、重置密码、完成引导三条「连点」测试写法相同，但各有「已经完成就不再做」的第二道防线，不会这样误报；它们是否每次都测到了重叠，不保证，这次没改。
