...
| #208 | `stack-2-core` `5a881c0` | 在（两次修复都落在这一层：`059db25`、`e6a8eb8`） | `5a881c0` 通过（run 35927282455，含「停掉媒体服务器后守卫报红」的对照）。这一层本轮新增：CI 控制步骤和机器记录（`7f30afa`）、视频测试测量（`3f3fb95`）、测试超时只在计时器就位后放行（`5a881c0`） |
| #209 | `stack-3-models` `6965599` | 在（从 stack 2 合并上来） | `6965599` 通过（run 35927281664，含对照）。这一层本轮新增：宠物替身加锁（`352fd8e`） |
| #210 / #204 | `feature/ios-native-prototype` `3337439` | 在 | `3337439` 两次都通过（run 35927282957、35927281157，含对照）。之前：`44f8d99` 两次里 1 次测试进程崩溃、`3b4f497` 两次里 1 次点赞测试失败，原因都已查明并修复，见「CI 上的另外两类红」 |
