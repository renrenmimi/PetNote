...
/// id, which is a fresh UUID per pick, so nothing here can go stale.
actor ComposeFilterPreviews {
    private let cache = NSCache<NSString, UIImage>()
    /// Decodes in flight, so concurrent callers for one photo share one.
    private var decoding: [String: Task<UIImage?, Never>] = [:]
    ...
    /// edge. Nil when it cannot be decoded or rendered.
    func preview(
    ...
    private func base(of item: ComposeViewModel.PickedItem, maxPixelSize: CGFloat) async -> UIImage? {
        ...
        if let cached = cache.object(forKey: key as NSString) { return cached }
        if let pending = decoding[key] { return await pending.value }
