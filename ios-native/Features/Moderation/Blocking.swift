...
actor BlockFilteringFeed: FeedRepository {
    ...
    /// it is out waits for it rather than asking again: a pull to refresh
    /// while another page read is out and the list is not in the cache — just
    /// after a block, or after a read of it failed — was two reads of the same
    /// list. Its answer is nil when the read failed — which fills no cache, so
